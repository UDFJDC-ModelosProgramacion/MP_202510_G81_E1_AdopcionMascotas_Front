...
import { ArrowLeft, MapPin, Phone, Mail, Calendar, Heart, Users, PawPrint } from 'lucide-react'
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-gray-600">Cargando información del refugio...</p>
        </div>
      </div>
    )
  }
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="flex flex-col md:flex-row gap-6">
            <div className="flex-shrink-0">
              <img
                src={shelter.image || '/placeholder-shelter.jpg'}
                alt={shelter.name}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-5">
            <TabsTrigger value="pets">Mascotas ({pets.length})</TabsTrigger>
            <TabsTrigger value="veterinarians">Veterinarios ({veterinarians.length})</TabsTrigger>
            <TabsTrigger value="events">Eventos ({events.length})</TabsTrigger>
            <TabsTrigger value="arrivals">Llegadas ({arrivals.length})</TabsTrigger>
            <TabsTrigger value="success">Historias ({successStories.length})</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="pets" className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {pets.map((pet) => (
                <Card key={pet.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                  ...
                  <CardHeader>
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <CardDescription>{pet.breed} • {calculateAge(pet.birthDate)} años</CardDescription>
                      </div>
                      <Badge variant="secondary">
                        Disponible
                      </Badge>
                  ...
                  <CardContent>
                    <p className="text-sm text-gray-600 mb-4">{pet.behaviorProfile}</p>
            ...
            {pets.length === 0 && (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500">No hay mascotas registradas en este refugio.</p>
              </div>
            )}
          </TabsContent>
          ...
          <TabsContent value="veterinarians" className="space-y-4">
            ...
            {veterinarians.length === 0 && (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500">No hay veterinarios registrados en este refugio.</p>
              </div>
            )}
          </TabsContent>
          ...
          <TabsContent value="events" className="space-y-4">
            ...
            {events.length === 0 && (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500">No hay eventos programados en este refugio.</p>
              </div>
            )}
          </TabsContent>
          ...
          <TabsContent value="arrivals" className="space-y-4">
            <div className="space-y-4">
              {arrivals.map((arrival) => (
                <Card key={arrival.id} className="hover:shadow-lg transition-shadow">
                  <CardContent className="pt-6">
                    <div className="flex items-start gap-4">
                      <div className="flex-shrink-0">
                        <img
                          src={arrival.petImage || '/placeholder-pet.jpg'}
                          alt={arrival.petName}
                      ...
                      <div className="flex-1">
                        <div className="flex justify-between items-start mb-2">
                          <h3 className="font-semibold text-lg">{arrival.petName}</h3>
                          <Badge variant="secondary">
                            {new Date(arrival.arrivalDate).toLocaleDateString('es-ES')}
                          </Badge>
                        </div>
                        <p className="text-sm text-gray-600 mb-2">
                          <strong>Rescatado por:</strong> {arrival.rescuer}
                        </p>
                        <p className="text-sm text-gray-600">{arrival.notes || 'Sin información adicional'}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
            ...
            {arrivals.length === 0 && (
              <div className="text-center py-8">
                ...
                <p className="text-gray-500">No hay registros de llegadas recientes.</p>
              </div>
            )}
          </TabsContent>
          ...
          <TabsContent value="success" className="space-y-4">
              ...
              {successStories.map((story) => (
                <Card key={story.id} className="hover:shadow-lg transition-shadow">
                  <CardHeader>
                    <div className="flex items-center gap-4">
                      <img
                        src={story.image || '/placeholder-pet.jpg'}
                        alt={story.petName}
                        className="w-16 h-16 rounded-lg object-cover"
                      />
                      <div>
                        <CardTitle className="text-lg">{story.petName}</CardTitle>
                        <CardDescription>
                  ...
                  <CardContent>
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-xs text-gray-500">
                        {story.adoptionDate ? new Date(story.adoptionDate).toLocaleDateString('es-ES') : 'Fecha no disponible'}
                      </span>
                    </div>
                  </CardContent>
                </Card>
            ...
            {successStories.length === 0 && (
              <div className="text-center py-8">
                <Heart className="mx-auto h-12 w-12 text-gray-400 mb-4" />
                <p className="text-gray-500">No hay historias de éxito registradas aún.</p>
              </div>
            )}
          </TabsContent>
        </Tabs>
